...
async def post_poll_vote(poll_id: ObjectIdType, ballot: PostBallot,
...
@router.get("/{poll_id}/voter_status", dependencies=[Depends(discord.requires_authorization)],
            response_model=TempVoterStatus)
async def temp_voter_status(poll_id: ObjectIdType, current_user: Annotated[dict, Depends(get_current_user)]):
    ...
    if voter is None:
        if poll.dynamic_voters is True:
            user_search = await users.find_one(
                {"_id": current_user["_id"]} | poll.voter_filter.model_dump(exclude_none=True))
